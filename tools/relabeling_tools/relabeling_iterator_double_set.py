...
for i in range(iteration):
    ...
    create_groundtruth_database('Custom3DDataset', 'data/rf2021', 'rf2021_seq', dir_path + cur_small_train2)


    print(f"==============Second Training of iteration {i+1}==============")
    train_message = \
        f"./tools/dist_train.sh {config} 2 --training-pkl {'relabeling_results_double_set/' + cur_train2} --no-validate"
    move_checkpoint_message = \
    ...
    move_train_pkl_message = \
        f"mv {dir_path + cur_train1[:-4] + '_changed_pred.pkl'} {dir_path + next_train1}"
    move_small_train_pkl_message = \
        f"mv {dir_path + cur_small_train1[:-4] + '_changed_pred.pkl'} {dir_path + next_small_train1}"
